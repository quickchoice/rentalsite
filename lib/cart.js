...
export const DEFAULT_EXPEDITE_FEE = 15;
...
export function isExpeditedOrder(orderMeta, now = new Date()) {
  ...
  const today = parseDateLocal(formatDateLocal(now));
  const dayDiff = Math.round((start.getTime() - today.getTime()) / ONE_DAY_MS);
...
export function getTaxAmount(subtotal) {
